...
class Options(ttk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.default_options = {
            ...
            "do_anti_afk": True,                        # Whether or not to do the anti afk measure
            "forages_per_anti_afk": 50,                 # How many forages it takes to do the anti afk
        }
        ...
        self.create_anti_afk_frame()
        self.create_forages_per_anti_afk_frame()
    ...
    def create_anti_afk_frame(self):
        self.anti_afk_frame = ttk.Frame(self)

        self.master.options["do_anti_afk"].set(self.default_options["do_anti_afk"])

        self.anti_afk_checkbutton = ttk.Checkbutton(
            self.anti_afk_frame,
            text="Enable Anti-AFK",
            variable=self.master.options["do_anti_afk"],
        )
        self.anti_afk_checkbutton.pack(side="bottom")

        self.anti_afk_frame.grid(row=0, column=1, padx=10, pady=10)
    
    def create_forages_per_anti_afk_frame(self):
        self.forages_per_anti_afk_frame = ttk.Frame(self)

        self.master.options["forages_per_anti_afk"].set(self.default_options["forages_per_anti_afk"])

        int_validate_cmd = self.register(self.validate_int_input)

        self.forages_per_anti_afk_entry = ttk.Entry(
            self.forages_per_anti_afk_frame,
            validate="key",
            validatecommand=(int_validate_cmd, "%P"),
            font=("", 8),
            textvariable=self.master.options["forages_per_anti_afk"],
        )
        self.forages_per_anti_afk_entry.pack(side="bottom")

        ttk.Label(self.forages_per_anti_afk_frame, text="Forages between Anti-AFK").pack(side="top")

        self.forages_per_anti_afk_frame.grid(row=1, column=1, padx=10, pady=10)
...
class App(ttk.Window):
    def __init__(self, *args, **kwargs):
        ...
        self.options = {
            ...
            "do_anti_afk": ttk.BooleanVar(self),   # Whether or not to jump every 20 forage operations
            "forages_per_anti_afk": ttk.StringVar(self),    # How many forages it takes to do an afk jump
        }
    ...
    def loop(self):
        if self.started:
            ...
            if self.forage_count % int(self.options["forages_per_anti_afk"].get()) == 0 and self.options["do_anti_afk"].get():
                anti_afk()
